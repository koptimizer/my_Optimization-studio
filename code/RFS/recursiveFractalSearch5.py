...
def randomBig(ranges) :
    randomList = []
    randomList += random.sample(range(0,ranges),ranges-2)
    return randomList
...
def chaosMove(nest, pointList) :
    nest = nest[:]
    for po in range(0, len(pointList), 2) :
        if not po >= len(pointList)-1 :
            nest = optSwap(nest, pointList[po], pointList[po+1])
    new_nest = nest
    return new_nest

def makeFractal(route) :
    global population
    tmp = population
    tmps = []
    for calls in range(Fractal_size) :
        for seq in range(len(tmp)) :
            tmps.append(doublebridgeMove(tmp[seq], sorted(randomFour(cities_count))))
            population.append(twoOptMove(tmp[seq], sorted(randomFour(cities_count))))
        population += tmps
        tmp = tmps
        tmps = []
...
def makePlot() :
    plotFrame = pd.DataFrame([gen, fit], columns=['gen', 'fit'])
    sns.scatterplot(x="gen", y='fit', data=plotFrame)
