...
def makeFractal(route, calls) :
    ...
    if not calls > Fractal_size:
        calls += 1
        if math.ceil(math.pow(random.uniform(0.0001,0.9999),-1.0/3.0)) <= 2 :
            new_route = twoOptMove(route, sorted(randomTwo(cities_count)))
        else :
            new_route = doublebridgeMove(route, sorted(randomFour(cities_count)))

        population.append(new_route)
        makeFractal(new_route, calls)
