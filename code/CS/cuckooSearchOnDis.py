...
if __name__ == "__main__" :
    cs = CS(nestCount=20, pa=0.25, pc=0.6, problem="cycle51.in")
    cs.search()
    plotData = []
    for index in cs.population[0, 0]:
        plotData.append([round(float(cs.dots_list[int(index)].split(" ")[0]), 3),
                         round(float(cs.dots_list[int(index)].split(" ")[1]), 3)])
    plotData = np.array(plotData)
    plotData = plotData.T

    textStr = "fitness :", cs.population[0, 1]

    axs = plt.subplot(plotData[0], plotData[1])
    plt.text(0.05, 0.95, textStr, fontsize=20, fontweight='bold')
    plt.show()

    print(cs.generation, '세대 최적 해 : \n', cs.population[0, 1])
    print(cs.population[0, 0])
    print(cs.stop - cs.start)
