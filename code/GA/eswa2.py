...
class GA :
    ...
    def encoding(self, chromosome):
        ...
        for i, j in zip(sequencer, chromosome[1]) : # 1~5, 1~2
            ...
            if os[j][i] == 0 and oss[j] == 0 and picker != j:
                os[j][i] += temp
                oss[j] += temp
            ...
            if os[j][i] == 0 and oss[j] != 0 and picker == j:
                ...
                oss[j] += temp
                machine[chromosome[0][i] + countNone].append(temp)
            elif os[j][i] == 0 and oss[j] != 0 and picker != j :
                os[j][i] += temp
                machine[chromosome[0][i] + countNone].append(oss[j])
                machine[chromosome[0][i] + countNone].append(temp)
                oss[j] += temp
            elif os[j][i] != 0 and oss[j] != 0:
                if os[j][i] <= oss[j] :
                    machine[chromosome[0][i] + countNone].append(oss[j]-os[j][i])
                    machine[chromosome[0][i] + countNone].append(temp)
                else :
                    machine[chromosome[0][i] + countNone].append(oss[j])
                    machine[chromosome[0][i] + countNone].append(temp)
