...
def makeFractal(route, calls) :
    ...
    if not calls > Fractal_size :
        ...
        chaos = chaosMove(route, sorted(randomBig(cities_count)))
        population.append(chaos)
        makeFractal(chaos, calls)
...
def run() :
    global population
    generation = 0
    optSol = optFunc(random.sample(range(0, cities_count), cities_count))
